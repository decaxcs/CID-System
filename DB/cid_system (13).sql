...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `update_cid_status_event` ON SCHEDULE EVERY 1 DAY STARTS '2024-05-15 14:34:37' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    -- Update cid_status in cs_cid_information based on the conditions
    UPDATE cs_cid_information AS i
    JOIN (
        SELECT cid_number,
               MAX(cid_sop_warranty_end) AS max_warranty_end
        FROM cid_summary_of_payments
        GROUP BY cid_number
    ) AS s
    ON i.cid_number = s.cid_number
    SET i.cid_status = 
        CASE
            WHEN s.max_warranty_end < CURRENT_DATE() AND s.max_warranty_end IS NOT NULL THEN 'Completed'
            WHEN s.max_warranty_end >= CURRENT_DATE() THEN 'On-going Warranty'
            ELSE i.cid_status -- Keep the current status if warranty_end is NULL
        END
    WHERE i.cid_status != 'Completed'; -- Only update if status is not already 'Completed'
END$$
